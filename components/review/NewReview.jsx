...
const NewReview = ({ movie }) => {
  ...
  const { error, clearErrors, postReview } = useContext(MovieContext);

  useEffect(() => {
    // const userReview = getUserReview(movie?.reviews, user?.user?.id);
  ...
  const submitHandler = () => {
    const reviewData = { rating, comment, movieId: movie?.id };
